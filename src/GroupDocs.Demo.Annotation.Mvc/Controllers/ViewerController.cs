...
using WatermarkPosition = GroupDocs.Demo.Annotation.Mvc.Models.WatermarkPosition;
...
namespace GroupDocs.Demo.Annotation.Mvc.Controllers
{
    public class ViewerController : Controller
    {
        ...
        public ViewerController()
        {
            ...
            //Here you should provide proper GroupDocs.Viewer license (in case you want to
            //use this sample without trial limits)
            //License lc = new License();
            //lc.SetLicense(@"C:\Users\w\Documents\GroupDocs.Viewer.lic");

            _htmlHandler = UnityConfig.GetConfiguredContainer().Resolve<ViewerHtmlHandler>();
            _imageHandler = UnityConfig.GetConfiguredContainer().Resolve<ViewerImageHandler>();
        }
        ...
        public ActionResult ViewDocument(ViewDocumentParameters request)
        {
            ...
            {
                DocumentInfoContainer docInfo = _imageHandler.GetDocumentInfo(new DocumentInfoOptions(request.Path));
                result.documentDescription = new FileDataJsonSerializer(docInfo.Pages, new FileDataOptions {UsePdf = true}).Serialize(true);
                result.docType = docInfo.DocumentType;
                result.fileType = docInfo.FileType;
        ...
        {
            var path = UnityConfig.GetConfiguredContainer().Resolve<ViewerConfig>().StoragePath;
